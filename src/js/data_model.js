...
var DataModel = function(bus_routes_data, bus_stops, map_objects, max_walking_distance_meters) {
    ...
    //bus_routes_daat, bus_stops and map_objects come from data scripts 
    //generated by Python utility. 
    //max_walking_distance_meters is a tuning variables - it 
    //represents max distance at which it is still considered o'k to walk from one object to another
    //decorate objects accordingly 
    ...
    //the idea is that within this application all we care about 
    ...
    //add extra fields to map objects - bus stops and destinations. 
    ...
    //There is one reachable object for each  source. 
    //it contains a list of objects that can be reached 
    //by taking a bus and walking.
    //i.e., destination object is reachable form source 
...
DataModel.prototype.get_map_center_coordinates = function() {
    //used during initialization to center map better (ensure all sources and destinations are shown at 
    //each step)
    var min_lat = 1000.0;
    var min_lng = 1000.0;
    var max_lat = -1000.0;
...
DataModel.prototype.get_map_objects = function(filter) {
    //reruns map objects that match the filter (i.e. each property of filter matches 
...
DataModel.prototype.decorate_map_objects = function(objects) {
    for (var i = 0, len = objects.length; i < len; i++) {
        ...
        //in this version only communities can be sources 
...
DataModel.prototype.build_local_distance_matrix = function() {
    ...
    //Thus, to minimize (or exclude) requests to maps API for estimating distances, 
    //  , 
    //the idea is to use locally built matrix for 
    //estimated distances and then process the locations that are close
    //(no point to calculate exact walking distance for points that are >2000 m away anyway)
    // 
